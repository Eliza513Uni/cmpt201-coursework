...
#define MAX_FILE_SIZE 512
  ...
  if (!(mdctx = EVP_MD_CTX_create())) {
    handle_error("unable to create mdctx");
  }

  /* Initialize `key` with a public key */
  if (1 != EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey)) {
    handle_error("Error with digest verify init");
  }

  /* Initialize `key` with a public key */
  if (1 != EVP_DigestVerifyUpdate(mdctx, message, messageBytes)) {
    handle_error("Error with digest verify update");
  }
  printf("DigestVerifyUpdate done\n");

  if (1 == EVP_DigestVerifyFinal(mdctx, signature, signatureBytes)) {
    /* Success */

...
1. How is the client sending data to the server? What protocol?
  The client is sending data via IPv4 using TCP because it's using a
  SOCK_STREAM object as opposed to SOCK_DGRAM
2. What data is the client sending to the server?
  It's sending short string messages (ie: "Dog")
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  run_acceptor is passed the necessary things to run a thread to handle a new
client: a bool indicating whether to continue running, and pointers to things to
handle the storing of messages
2. How are received messages stored?
  They are stored in a struct list_handle that is accessed via pointer and
controlled by a mutex
3. What does `main()` do with the received messages?
  It checks if enough messages were received and collected, and gives a
corresponding print output
4. How are threads used in this sample code?
  Each socket for handling a single client gets its own thread to run on

5. Sockets are made non-blocking using the set_non_blocking() function which
uses fcntl() to mark a socket as non-blocking
The sockets made non-blocking are
the ones used for the threads each managing a single client
The sockets are made
non-blocking so that nothing halts while waiting on client input and the thread
can keep trying to read input
*/
